...
__attribute__ ((naked)) void SysTick_Handler() {
	 ...
	 *
	 * explanation for the instructions below:
	 * Load: It loads a 32-bit word from the memory address stored in R0 into R41.
	 * Post-indexing: After the load operation, it increments the value in R0 by 4 bytes (1 word)3.
	 * */
...
void init_task_stacks() {
	...
	for (int i = 0; i < MAX_TASKS; i++) {
		pPSP = (uint32_t*) user_tasks[i].psp_value; // gets the current task's stack start
